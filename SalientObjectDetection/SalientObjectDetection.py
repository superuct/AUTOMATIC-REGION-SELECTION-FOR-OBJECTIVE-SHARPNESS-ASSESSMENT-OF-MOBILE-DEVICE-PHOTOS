...
def SalientObjectDetection(rgb):
	...
	# args.gpu_fraction = 1.0
	
	gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction = 1.0)#args.gpu_fraction)
	with tf.Session(config=tf.ConfigProto(gpu_options = gpu_options)) as sess:
		saver = tf.train.import_meta_graph('./SalientObjectDetection/meta_graph/my-model.meta')
		saver.restore(sess,tf.train.latest_checkpoint('./SalientObjectDetection/salience_model'))
